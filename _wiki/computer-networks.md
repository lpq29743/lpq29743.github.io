...
- URG：长度为 1 bit，标识紧急指针是否有效
- ACK：长度为 1 bit，标识确认号是否有效
- PSH：长度为 1 bit，标识接收方在接收到该报文段后，是否立刻交付给应用层，而不在缓冲区排队
- RST：长度为 1 bit，标识是否重置连接
- SYN：长度为 1 bit，标识是否发起连接
- FIN：长度为 1 bit，表示是否已完成发送任务
