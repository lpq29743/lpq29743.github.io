...
6. 有两个未知整数，你可以不断询问某个数与这两个数的大小关系（每次询问一个数），该如何查找这两个数？
...
10. 什么是深度学习？答：深度学习是机器学习的一种，其目标是试图用神经网络模拟大脑的神经元活动，用更深的网络来模拟人的思考。
...
16. 常见的距离度量方法有哪些？
    ...
    答：$$L_p$$ 距离 / Minkowski 距离 / 闵式距离是最常规的距离度量方式，其公式为 $$(\left|x-y\right|^p)^{1/p}$$。当 $$p = 1$$ 时为曼哈顿距离，$$p = 2$$ 时为欧式距离，$$p​$$ 为无穷大时为各个坐标距离的最大值，即切比雪夫距离。
...
20. 当数据线性可分、接近线性可分以及线性不可分时，分别使用什么 SVM？
...
22. 手推 SVM
    ...
    样本点 $$P(x_i, y_i)$$ 到超平面的几何距离：$$\frac{\left|w^Tx_i+b\right|}{\|w\|}$$；
...
29. 给一组数据，问决策树、LR、NB 以及 SVM 等算法学出来是什么样子的？
...
36. 上溢和下溢是什么，softmax 函数会出现哪种情况，该怎么解决？
...
43. sgd、momentum、rmsprop、adam 区别与联系
...
76. Glove 和 LSA 有什么联系或区别？

    答：LSA  采用计算复杂度高的 SVD 算法，Glove 可看作是对 LSA 的一种优化。

77. Glove 和 word2vec 有什么联系或区别？
