...
7. 给一个有 m 个样本，n 维特征的数据集，如果用 LR 算法，那么梯度是几维？
...
32. 当数据线性可分、接近线性可分以及线性不可分时，分别使用什么 SVM？
...
41. 给一组数据，问决策树、LR、NB 以及 SVM 等算法学出来是什么样子的？
...
54. 上溢和下溢是什么，softmax 函数会出现哪种情况，该怎么解决？
...
55. 讲一下 EM 算法，E 步和 M 步的具体步骤，E 中的期望是什么？
...
81. sgd、momentum、rmsprop、adam 区别与联系
...
83. TensorFlow 内部求导机制？

    答：符号求导。先提供每一个op求导的数学实现，然后使用链式法则求出整个表达式的导数。

84. TensorFlow 创建变量的方式有哪些，有什么区别？
...
103. CNN 在文本分类上一般怎么应用？

     答：卷积核宽度为词向量的维度，长度为关注窗口的大小。

104. CNN 在卷积和池化过程中，输入特征和输出特征的关系是怎样的？

     答：输出尺寸 = (输入尺寸 - filter + 2 * padding）/ stride + 1。计算尺寸不被整除，卷积向下取整，池化向上取整。

105. 为什么在 CNN 等结构中将原先的 sigmoid、tanh 换成 ReLU 可以取得比较好的效果？
...
108. RNN 中只能采用 tanh 而不是 ReLU 作为激活函数么？
...
137. Glove 和 LSA 有什么联系或区别？

     答：LSA  采用计算复杂度高的 SVD 算法，Glove 可看作是对 LSA 的一种优化。

138. Glove 和 word2vec 有什么联系或区别？
...
145. Transformer 的 position encoding 为什么选三角函数？
...
147. Transformer 使用的时候，制约显存的最关键因素是什么？
...
166. max pooling 和 mean pooling 是什么？有其他方式吗？
