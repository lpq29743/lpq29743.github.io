...
31. 当数据线性可分、接近线性可分以及线性不可分时，分别使用什么 SVM？
...
40. 给一组数据，问决策树、LR、NB 以及 SVM 等算法学出来是什么样子的？
...
53. 上溢和下溢是什么，softmax 函数会出现哪种情况，该怎么解决？
...
54. 讲一下 EM 算法，E 步和 M 步的具体步骤，E 中的期望是什么？
...
80. sgd、momentum、rmsprop、adam 区别与联系
...
82. TensorFlow 内部求导机制？

    答：符号求导。先提供每一个op求导的数学实现，然后使用链式法则求出整个表达式的导数。

83. Pytorch 如何切换训练和测试模式？
...
101. CNN 在文本分类上一般怎么应用？

     答：卷积核宽度为词向量的维度，长度为关注窗口的大小。

102. CNN 在卷积和池化过程中，输入特征和输出特征的关系是怎样的？

     答：输出尺寸 = (输入尺寸 - filter + 2 * padding）/ stride + 1。计算尺寸不被整除，卷积向下取整，池化向上取整。

103. 为什么在 CNN 等结构中将原先的 sigmoid、tanh 换成 ReLU 可以取得比较好的效果？
...
106. RNN 中只能采用 tanh 而不是 ReLU 作为激活函数么？
...
135. Glove 和 LSA 有什么联系或区别？

     答：LSA  采用计算复杂度高的 SVD 算法，Glove 可看作是对 LSA 的一种优化。

136. Glove 和 word2vec 有什么联系或区别？
...
143. Transformer 的 position encoding 为什么选三角函数？
...
145. Transformer 使用的时候，制约显存的最关键因素是什么？
...
164. max pooling 和 mean pooling 是什么？有其他方式吗？
...
7. C++ 中的 public、private 和 protected 是什么？
...
5. Python 中怎么转换 ascii 码和字符？

      答：`chr(65)`和`ord('A')`。

6. `len('中文')`和`len('中文'.encode('utf-8'))`的输出分别是多少？

      答：2 和 6，Python 默认是  Unicode 编码，转换成 UTF-8 编码，中文通常占用三个字符，英文一个。

7. 一行代码将字符串 "->" 插入到 "abcdefg" 中每个字符的中间
...
9. @staticmethod 和 @classmethod 的作用与区别

      答：[链接](https://blog.csdn.net/qq_15037231/article/details/77943109)

10. Python 的 getter 和 setter 是什么？
