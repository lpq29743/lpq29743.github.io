...
19. 高维空间下的点有什么特点？
    ...
    答：1. 随着维度升高，点的分布会变得稀疏，任意两点的距离会变大。
    2. 距离会变得差不多。距离的相对方差为 1/d，d 为维度，随着维度升高，方差趋于 0。
    3. 集中在表面。从低维推到高维空间，体积会越来越由表面附近区域贡献。
    4. 几乎正交
...
5. 链表
    ...
    - 链表是否有环：可以使用快慢指针（Floyd判圈算法），慢指针每次走一步，快指针每次走两步，如果链表有环，两者最终会相遇。时间复杂度为 O(n)，空间复杂度为 O(1)。
    - 链表环的入口节点：相遇后一指针回头从头出发，再次相遇点即入口。
...
7. 队列
    ...
    答：- 用栈实现队列：用两个栈模拟队列的入队和出队操作，实现FIFO行为。
...
8. 树
    ...
    答：- 二叉树的前序、中序、后序遍历：递归或迭代实现节点访问顺序。
...
25. 从一个几乎排序好的数组中找出第 k 小的元素，时间复杂度尽量低。  

    答：利用“插入排序”特性，数组接近有序，插入排序接近线性。也可以用快速选择算法，平均 O(n)。

26. 在 n 个数中取最大的 k 个数，时间复杂度是？

    答：nlogk。堆的大小为 k，总共要调整 n 次。

27. 有 10 个排好序的数据库，那么我要找整个的中位数，怎么找？
...
39. 有两个未知整数，你可以不断询问某个数与这两个数的大小关系（每次询问一个数），该如何查找这两个数？

    答：[链接](https://www.zhihu.com/question/310970538)

40. 一群木板，一开始有一条线把它们固定在一条水平线上，现在抽掉这条线，有的木板往下掉落，有的木板位置上升，问怎么移动才能使移动距离最小，让它们继续在一条水平线上？

    答：中位数。

41. 给定两个数，求他们无限次相加中第 k 小的数？
...
11. 给一个有 m 个样本，n 维特征的数据集，如果用 LR 算法，那么梯度是几维？
...
14. 如何对单层感知机进行改进，使其能够解决异或问题？

    答：多层感知机，或在进入激活函数前加一个多项式模块，从而添加非线性成分。

15. 怎么判断分类器是线性分类器还是非线性分类器？
...
42. 用 numpy 实现 cross entropy loss
