...
感知机分类器用来解决绝对线性可分的问题，其主要目的是为了训练得到一个超平面，这个超平面可用公式 $$wx + b$$  表示，训练结果就是为了得到一个可保证两个类别分居在超平面两侧、无一样本被错误分类的 $$w$$ 和 $$b$$。具体的训练方法主要有两种：一种是传统的方法，只考虑 $$w$$，根据每一个误分类点调整 $$w$$，这里我们不讲解这一种方法，另一种是从优化损失函数的角度思考。

我们可以用 $$+1​$$ 和 $$-1​$$ 来分别表示正类和负类，用 $$(x_i, y_i)​$$ 表示样本 $$ i ​$$，因此我们可以推出如果样本被误分类，则有 $$-y_i(wx_i + b) > 0​$$。所以我们可以得到损失函数 $$ L = -\sum_{x_i \in M}y_i(wx_i + b) ​$$，其中 $$M​$$ 表示误分类集合。通过梯度下降算法优化损失函数，就可以得到满意的参数值。这里值得注意的是，由于这是完全线性可分的问题，所以要求最终 $$L = 0​$$。

感知器分类器得到的超平面并不能最好地体现分类情况，因此 SVM 引入了硬间隔最大化的概念。

通过硬间隔最大化能够得到一个线性 SVM

情况二：当训练样本只是接近线性可分

需要引入松弛变量，通过软间隔最大化，得到线性 SVM
